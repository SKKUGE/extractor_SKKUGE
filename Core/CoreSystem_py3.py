...
class Helper(object):
    ...
    def CheckSameNum(strInputProject, listSamples):
        ...
        listProjectNumInInput = [i for i in sp.check_output(f'ls {strInputProject}', shell=False).split('\n') if
                                 i != '']
        ...
        if intProjectNumInTxt != len(setSamples - setProjectNumInInput):
            ...
            logging.warning(f'Input folder: {intProjectNumInInput}, Project list samples: {intProjectNumInTxt}')
    ...
    def CheckAllDone(strOutputProject, listSamples):
        intProjectNumInOutput = len([i for i in sp.check_output('ls %s' % strOutputProject, shell=False).split('\n') if
...
class InitialFolder(object):
    ...
    def MakeInputFolder(self):
        ## './Input/JaeWoo'
        strUserInputDir = f'./Input/{self.strUser}'
        ...
        ## './User/JaeWoo'
        strUserDir = f'./User/{self.strUser}'
        ...
        if not os.path.isfile(self.strProjectFile):
            # Replacing subprocess.call method with subprocess.run method
            # For long sustainability

            # https: // docs.python.org / 3 / library / subprocess.html  # security-considerations
            sp.run('> ' + self.strProjectFile, shell=False)

    def MakeOutputFolder(self):

        ## './Output/JaeWoo'
        strOutputUserDir = f'./Output/{self.strUser}'
        ...
        self.strLogPath = os.path.join(self.strOutputProjectDir, f'Log/{strLogName}_log.txt')
